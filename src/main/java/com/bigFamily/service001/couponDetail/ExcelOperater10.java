 ...
 */
@Component
public class ExcelOperater10 {
    @Autowired
    private RedisServiceImpl redisService;

    public void initCouponDetail(String tuyaCouponStatus) {
        ...
        try {
            ...
            String excelName = "1".equals(tuyaCouponStatus)?"(已使用)":"(未使用)";
            File filewrite = new File("/Users/shuistyanlong/Documents/work/workspace/yangyi/couponByTuya/7.用户领取优惠券数据"+excelName+".xls");
            ...
            for (int i = 0; i < rsRows; i++) {
                ...
                Cell cell009 = readsheet.getCell(10, i);
                String couponStatus0 = cell009.getContents().trim();
                if(!"1".equals(tuyaCouponStatus) && "1".equals(couponStatus0)){
                    continue;
                }else if("1".equals(tuyaCouponStatus) && !"1".equals(couponStatus0)) {
                    continue;
                }
                ...
                for (int j = 0; j < 17; j++) {
                    ...
                    if (temp == 3) {//user_id
                        Cell cell002 = readsheet.getCell(2, i);
                        String uid = cell002.getContents().trim();
                        String tuyaUserIdKey = CouponConstant.TUYPA_USER_ID+uid;
                        String itnodeUserInfo = (String) redisService.get(tuyaUserIdKey);
                        if(StringUtils.hasLength(itnodeUserInfo)){
                            val = itnodeUserInfo.split("&")[0];
                        }
                    }
                    if (temp == 4) {//unique_id
                        Cell cell002 = readsheet.getCell(2, i);
                        ...
                        String itnodeUserInfo = (String) redisService.get(tuyaUserIdKey);
                        if(StringUtils.hasLength(itnodeUserInfo)){
                            val = itnodeUserInfo.split("&")[1];
                        }
                    }
                    if (temp == 5) {//apply_time
                        Cell cell = readsheet.getCell(4, i);
                        String effectiveBeginTime = cell.getContents();
                        long tiemspan = Long.parseLong(effectiveBeginTime);
                        String result = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(tiemspan);
                        val = result;
                    }
                    if (temp == 6) {//use_time
                        Cell cell = readsheet.getCell(9, i);
                        String useTime = cell.getContents().trim();
                        if(StringUtils.hasLength(useTime)) {
                            long tiemspan = Long.parseLong(useTime);
                            String result = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(tiemspan);
                            val = result;
                        }else {
                            val = null;
                    ...
                    if (temp == 8) {//order_id
                        ...
                        String invitationCode = cell.getContents().trim();
                        String key = CouponConstant.INVITATION_CODE+invitationCode;
                        String orderId = (String) redisService.get(key);
                        val = orderId;


                        Cell cell001 = readsheet.getCell(1, i);
                        String tuyaCouponId = cell001.getContents();
                        String tuyaCouponIdKey = CouponConstant.TUYA_COUPON_ID_KEY+tuyaCouponId;
                        String itnodeCouponId = (String) redisService.get(tuyaCouponIdKey);
                        String tuyaCouponCodekey = CouponConstant.TUYA_COUPON_CODE+invitationCode;
                        redisService.set(tuyaCouponCodekey,itnodeCouponId);
                    }
                    if (temp == 9) {//deleted
                        val = "0";
                    }
                    ...
                    if (temp == 17) {//expire_time
                        ...
                        String invalidateTime = cell.getContents();
                        long tiemspan = Long.parseLong(invalidateTime);
                        if (tiemspan !=0) {
                            String result = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(tiemspan);
                            val = result;
                        }
